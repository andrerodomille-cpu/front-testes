...
type SingleSelectProps = {
  ...
  obrigatorio?: boolean;
  disabled?: boolean;
  onChange: (id: string, newValue: string | null) => void;
};

export const InputSelect: React.FC<SingleSelectProps> = ({
  id,
  value,
  label,
  options,
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className={`truncate ${!selectedLabel ? "text-muted-foreground" : ""}`}>
              {selectedLabel || t("comum.selecione")}
            </span>
            <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="bg-gray-50 dark:bg-gray-900 w-[var(--radix-popover-trigger-width)] p-2">
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-3 h-3 border rounded-full mr-2 flex items-center justify-center ${
                        isSelected
